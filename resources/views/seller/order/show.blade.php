...
@push('script')
    <script>
        ...
        function confirmCancel() {
            ...
            fetch('{{ route("seller.orders.updateStatus", ":id") }}'.replace(':id', currentOrderId), {
                method: 'PUT',
                headers: {
                ...
                body: JSON.stringify({
                    status: 'cancelled',
                    notes: reason || 'Order cancelled by seller'
                })
            })
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        setTimeout(() => {
                            window.location.href = '{{ route("seller.orders.cancelled") }}';
                        }, 1500);
                    } else {
        ...
        document.getElementById('statusForm').addEventListener('submit', function(e) {
            ...
            fetch('{{ route("seller.orders.updateStatus", $order->id) }}', {
                method: 'PUT',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Accept': 'application/json',
